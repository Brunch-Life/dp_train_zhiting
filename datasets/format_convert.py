...
load_dir = "/iag_ad_01/ad/tangyinzhou/tyz/observation-reward-model/data/world_model_rl_dataset/tomato_random/data"
save_dir = (
    "/iag_ad_01/ad/tangyinzhou/tyz/reward_diffusion_policy/diffusion_policy/data/pick"
)
os.makedirs(save_dir, exist_ok=True)
for episode_idx, episode in tqdm(
    enumerate(os.listdir(load_dir)), total=len(os.listdir(load_dir))
):
    data = np.load(
    ...
    action = np.concatenate(
        [
            data["action"]["end"]["position"],
            get_seq_euler_angles(data["action"]["end"]["orientation"]),
    ...
    desired_gripper_width = data["action"]["effector"]["position_gripper"].reshape(-1)
    # save_file = {
    #     "tcp_pose": tcp_pose,
